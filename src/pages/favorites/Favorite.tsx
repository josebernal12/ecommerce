...
const Favorite = ({ product }: Prop) => {
  ...
  useEffect(() => {
    ...
    if (favoriteLocalStorage) {
      const favoriteParse = JSON.parse(favoriteLocalStorage)
      setFavorite(favoriteParse)
    ...
    <div className=" rounded-lg border bg-card text-card-foreground shadow-sm" data-v0-t="card">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-1 justify-between">
          ...
          <svg onClick={() => handleFavorites(product)} xmlns="http://www.w3.org/2000/svg" fill={`${favorite.some(value => value.id === product.id) ? 'solid' : 'none'}`} viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6 hover:cursor-pointer">
